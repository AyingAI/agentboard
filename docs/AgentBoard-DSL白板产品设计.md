...
1. 白板主界面
   ...
   - 可以在画布空白处框选多个节点，并整体拖动，方便移动同一 group 内的一组卡片
...
Agent 生成节点时，第一眼布局必须可用：

- 新节点不能和已有节点堆叠或贴边。
- 新节点之间也不能互相遮挡。
- 默认至少保留 56px 视觉间距。
- 如果 Agent 给出的坐标过近，patch engine 需要自动只移动本次新增或几何变化节点，不移动用户原有布局。
...
1. 用户仍然在画板上用节点和连线设计流程。
2. 节点表达步骤、输入、决策、输出或检查点。
3. 连线表达执行顺序、依赖或条件。
4. 用户在底部对话框输入“执行流程”“执行这条流程”“从某个节点开始执行”。
5. Agent 读取当前 `BoardDSL`，把可视化流程解释成执行计划。
6. 如果信息不足，Agent 返回 `interaction_request`，在活动面板/确认浮窗里让用户补充。
7. 用户确认后，Agent 按流程执行，并把状态和结果回写到画板。

这里的关键不是让 Agent “看图执行”，而是让图上的节点、连线、分组和用户补充说明共同形成结构化执行上下文。
...
- 哪些节点是真任务，哪些节点只是说明。
- 最终产物是什么。
- 是否需要联网、读文件、调用外部系统或其它授权能力。
- 哪些步骤需要用户确认后才能继续。

如果这些信息不充分，Agent 不应该硬跑，而应该先返回确认请求。

确认请求不应泛泛地问“是否继续”，而应展示 Agent 已解析出的计划，例如：
...
- 是否允许联网搜索？
- 搜索资料是否需要真实引用来源？
- 自检是否按“用户价值 / 商业价值 / 技术风险”检查？
```

执行结果必须回到画板，而不是只留在聊天记录里：

- 原流程节点可被更新为 `待执行 / 执行中 / 已完成 / 需确认 / 失败`。
- 每个关键步骤旁边生成结果节点。
- 风险写成 `risk` note。
- 问题写成 `question` note。
- 最终结论写成 `output` 或 `action` 节点。
- 除非用户要求整理全图，否则不打乱原有流程布局。

第一版 MVP 不做“任意自动化平台”，只做这条最小闭环：
...
AgentBoard 需要让用户知道 Agent 正在做什么，否则执行型任务一旦超过几秒，用户会很容易怀疑系统卡住。

但这里不应该做成“完整推理链展示”。原因是：

- 完整推理链不稳定，也不一定适合直接暴露给用户。
- 用户真正需要的是进展、当前阶段、是否还在运行、是否需要自己介入。
- 企业场景更需要可审计的执行事件，而不是模型内部思考文本。

更合理的交互是：

1. Agent 运行时，在画板上显示一个临时运行浮层。
2. 浮层展示当前阶段、耗时、最近事件、可停止入口和查看详情入口。
3. 结果输出、失败、取消或转为确认请求后，浮层自动隐藏。
4. 完整事件留在右侧 `Agent Activity`，用于追溯和调试。

浮层内容只展示用户可理解、可审计的执行摘要，例如：
...
它的产品定位不是“看见 Agent 的脑子”，而是“看见 Agent 的工作状态”。
...
Agent 需要先把流程解释成执行计划。如果当前画板只有一条连通流程，且起点、终点、目标足够清楚，可以直接生成确认请求；如果画板上有多条流程、缺少目标或需要外部权限，则必须先追问。

这个场景的价值在于：用户不需要把流程重新写成 prompt，也不需要维护另一套 workflow 配置。白板本身就是执行上下文，聊天框只是触发入口和确认入口。
